...
cvfit <- cv.glmnet(x, survobj, family = "cox") #Cross-validation
...
The idea is to have a for loop. For each loop, randomly sample n predictors from all the predictors, subset the data with outcome and the selected n predictors, and then run cox regression using the subsetted data frame. You may record AIC of each random cox model for downstream comparison.
